...
#ifndef CPPA_IO_NETWORK_HPP
#define CPPA_IO_NETWORK_HPP
...
#if !defined(CPPA_LINUX) || defined(CPPA_POLL_IMPL) // poll() multiplexer
#   define CPPA_POLL_MULTIPLEXER
#   ifndef CPPA_WINDOWS
#       include <poll.h>
#   endif // CPPA_WINDOWS
#   ifndef POLLRDHUP
...
class multiplexer {
 ...
 private:
    ...
    void new_event(F fun, operation op, native_socket fd, event_handler* ptr) {
        ...
        if (i != last && i->fd == fd) {
            CPPA_REQUIRE(ptr == i->ptr);
            // squash events together
            CPPA_LOG_DEBUG("squash events: " << i->mask << " -> "
                           << fun(op, i->mask));
            auto bf = i->mask;
            i->mask = fun(op, bf);
            if (i->mask == bf) {
                // didn't do a thing
                CPPA_LOG_DEBUG("squashing did not change the event");
            } else if (i->mask == old_bf) {
                // just turned into a nop
                CPPA_LOG_DEBUG("squashing events resulted in a NOP");
                m_events.erase(i);
            }
        } else {
            // insert new element
            auto bf = fun(op, old_bf);
            if (bf == old_bf) {
                CPPA_LOG_DEBUG("event has no effect (discarded): "
                               << CPPA_ARG(bf) << ", " << CPPA_ARG(old_bf));
            } else {
...
class stream : public event_handler {
 ...
 public:
    ...
    void write(const void* buf, size_t num_bytes) {
        CPPA_LOG_TRACE("num_bytes: " << num_bytes);
    ...
    void flush(const manager_ptr& mgr) {
        CPPA_REQUIRE(mgr != nullptr);
        CPPA_LOG_TRACE("offline buf size: " << m_wr_offline_buf.size()
                       << ", mgr = " << mgr.get()
 ...
 private:
    ...
    void write_loop() {
        CPPA_LOG_TRACE("wr_buf size: " << m_wr_buf.size()
...
class acceptor : public event_handler {
 ...
 public:
     ...
     */
    void start(const manager_ptr& mgr) {
        CPPA_LOG_TRACE("m_accept_sock.fd = " << m_accept_sock.fd());
        CPPA_REQUIRE(mgr != nullptr);
        m_mgr = mgr;
