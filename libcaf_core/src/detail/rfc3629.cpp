...
// Returns a pair consisting of the last iterator to where the range is valid,
// and a boolean statin whether the validation failed because of incomplete
// data, or other failuers like malformed byte encoding.
  ...
  while (first != last) {
    ...
    if (head<3>(x) == 0b1100'0000_b) {
      ...
      if (head<7>(x) == 0b1100'0000_b)
        return std::pair(checkpoint, false);
      if (first == last)
        return std::pair(checkpoint, true);
...
auto rfc3629::validate(const_byte_span bytes) noexcept
  -> std::pair<size_t, bool> {
  auto [last, incomplete] = validate_rfc3629(bytes.begin(), bytes.end());
  return std::pair(last - bytes.begin(), incomplete);
}
