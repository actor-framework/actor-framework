...
TEST("swap exchanges the content of two expected") {
  ...
  SECTION("lhs: error, rhs: error") {
    ...
    lhs.swap(rhs);
    check_eq(lhs, caf::unexpected{error{sec::logic_error}});
    check_eq(rhs, caf::unexpected{error{sec::runtime_error}});
  }
...
TEST("expected is convertible from none") {
  e_int x{caf::unexpected{error{}}};
  if (check(!x))
    check(x.error().empty());
  e_void y{caf::unexpected{error{}}};
  if (check(!y))
    check(!y.error().empty());
}
...
TEST("and_then does nothing when called with an error") {
  SECTION("non-void value type") {
    ...
    check_eq(v1, caf::unexpected{error{sec::runtime_error}});
    check_eq(v2, caf::unexpected{error{}}); // has been moved-from
    check_eq(v3, caf::unexpected{error{sec::runtime_error}});
    check_eq(v4, caf::unexpected{error{sec::runtime_error}});
    check_eq(v5, caf::unexpected{error{sec::runtime_error}});
  }
  SECTION("void value type") {
...
struct next_error_t {
  ...
  auto operator()(error&& err) const {
    ...
    if constexpr (WrapIntoExpected)
      return expected<T>{caf::unexpected{std::move(err)}};
    else
      return std::move(err);
  }
};
...
TEST("or_else does nothing when called with a value") {
  SECTION("non-void value type") {
    auto uh_oh = [this](auto&& e) {
      fail("uh-oh called!");
      return e_iptr{caf::unexpected(e)};
    };
    auto i = make_counted<counted_int>(1);
    auto v1 = e_iptr{std::in_place, i};
