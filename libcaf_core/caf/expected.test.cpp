...
TEST("expected reports its status via has_value() or operator bool()") {
  ...
  check(x.has_value());
  e_int y{make_unexpected(sec::runtime_error)};
  check(!y);
  check(!y.has_value());
}
...
TEST("an expected exposed its value via value()") {
  ...
  SECTION("value() throws if has_value() would return false") {
    auto val = e_iptr{make_unexpected(sec::runtime_error)};
    const auto& cval = val;
...
TEST("value_or() returns the stored value or a fallback") {
  ...
  SECTION("lvalue access with error") {
    auto val = e_iptr{make_unexpected(sec::runtime_error)};
    auto k = counted_int_ptr{val.value_or(j)};
    check_eq(val, make_unexpected(error{sec::runtime_error}));
    check_eq(k, j);
  }
...
TEST("emplace destroys an old value or error and constructs a new value") {
  ...
  SECTION("void value type") {
    ...
    x.emplace();
    check(x.has_value());
    e_void y{make_unexpected(sec::runtime_error)};
    check(!y);
    y.emplace();
...
TEST("swap exchanges the content of two expected") {
  ...
  SECTION("lhs: value, rhs: error") {
    ...
    e_str rhs{make_unexpected(sec::runtime_error)};
    check_eq(lhs, "this is a value"s);
    check_eq(rhs, make_unexpected(error{sec::runtime_error}));
    lhs.swap(rhs);
    check_eq(lhs, make_unexpected(error{sec::runtime_error}));
    check_eq(rhs, "this is a value"s);
  }
  SECTION("lhs: error, rhs: value") {
    e_str lhs{make_unexpected(sec::runtime_error)};
  ...
  SECTION("lhs: error, rhs: error") {
    e_str lhs{make_unexpected(sec::runtime_error)};
    e_str rhs{make_unexpected(sec::logic_error)};
    check_eq(lhs, make_unexpected(error{sec::runtime_error}));
    check_eq(rhs, make_unexpected(error{sec::logic_error}));
    lhs.swap(rhs);
    check_eq(lhs, make_unexpected(error{sec::logic_error}));
    check_eq(rhs, make_unexpected(error{sec::runtime_error}));
  }
  ...
  SECTION("lhs: void, rhs: error") {
    e_void lhs;
    e_void rhs{make_unexpected(sec::runtime_error)};
    check(static_cast<bool>(lhs));
    check_eq(rhs, make_unexpected(error{sec::runtime_error}));
    lhs.swap(rhs);
    check_eq(lhs, make_unexpected(error{sec::runtime_error}));
  ...
  SECTION("lhs: error, rhs: void") {
    e_void lhs{make_unexpected(sec::runtime_error)};
    e_void rhs;
    check_eq(lhs, make_unexpected(error{sec::runtime_error}));
    check(static_cast<bool>(rhs));
    lhs.swap(rhs);
  ...
  SECTION("lhs: error, rhs: error") {
    e_void lhs{make_unexpected(sec::runtime_error)};
    e_void rhs{make_unexpected(sec::logic_error)};
    check_eq(lhs, make_unexpected(error{sec::runtime_error}));
    check_eq(rhs, make_unexpected(error{sec::logic_error}));
    lhs.swap(rhs);
    check_eq(lhs, make_unexpected(error{sec::logic_error}));
...
TEST("an expected can be compared to its expected type and errors") {
  SECTION("non-void value type") {
    ...
    check_eq(x, 42);
    check_ne(x, 24);
    check_ne(x, make_unexpected(sec::runtime_error));
    e_int y{make_unexpected(sec::runtime_error)};
    check_ne(y, 42);
    check_ne(y, 24);
    check_eq(y, make_unexpected(sec::runtime_error));
    check_ne(y, make_unexpected(sec::logic_error));
  }
  SECTION("void value type") {
    e_void x;
    check(static_cast<bool>(x));
...
TEST("two expected with the same error are equal") {
  SECTION("non-void value type") {
    e_int x{make_unexpected(sec::runtime_error)};
    e_int y{make_unexpected(sec::runtime_error)};
    check_eq(x, y);
    check_eq(y, x);
  }
  SECTION("void value type") {
    e_void x{make_unexpected(sec::runtime_error)};
    e_void y{make_unexpected(sec::runtime_error)};
    check_eq(x, y);
    check_eq(y, x);
  }
...
TEST("two expected with different errors are unequal") {
  SECTION("non-void value type") {
    e_int x{make_unexpected(sec::logic_error)};
    e_int y{make_unexpected(sec::runtime_error)};
    check_ne(x, y);
    check_ne(y, x);
  }
  SECTION("void value type") {
    e_void x{make_unexpected(sec::logic_error)};
    e_void y{make_unexpected(sec::runtime_error)};
    check_ne(x, y);
    check_ne(y, x);
  }
...
TEST("expected is convertible from none") {
  e_int x{make_unexpected(error{})};
  if (check(!x))
    check(x.error().empty());
  e_void y{make_unexpected(error{})};
  if (check(!y))
    check(!y.error().empty());
}
...
TEST("and_then does nothing when called with an error") {
  SECTION("non-void value type") {
    ...
    auto v1 = e_iptr{make_unexpected(sec::runtime_error)};
    ...
    check_eq(v1, make_unexpected(error{sec::runtime_error}));
    check_eq(v2, make_unexpected(error{})); // has been moved-from
    check_eq(v3, make_unexpected(error{sec::runtime_error}));
    check_eq(v4, make_unexpected(error{sec::runtime_error}));
    check_eq(v5, make_unexpected(error{sec::runtime_error}));
  }
  SECTION("void value type") {
    auto fn = [] { return e_void{}; };
    auto v1 = e_void{make_unexpected(sec::runtime_error)};
...
struct next_error_t {
  auto operator()(const error& err) const {
    ...
    if constexpr (WrapIntoExpected)
      return expected<T>{make_unexpected(static_cast<sec>(code))};
    else
  ...
  auto operator()(error&& err) const {
    ...
    if constexpr (WrapIntoExpected)
      return expected<T>{make_unexpected(std::move(err))};
    else
      return std::move(err);
  }
};
...
TEST("or_else may replace the error or set a default") {
  SECTION("non-void value type") {
    ...
    SECTION("or_else makes copies when called on a mutable lvalue") {
      auto v1 = e_int{make_unexpected(sec::runtime_error)};
      auto v2 = v1.or_else(next_error);
      check_eq(v1, make_unexpected(sec::runtime_error));
      check_eq(v2, make_unexpected(sec::remote_linking_failed));
      auto v3 = v2.or_else(set_fallback);
      check_eq(v2, make_unexpected(sec::remote_linking_failed));
      check_eq(v3, 42);
    }
    SECTION("or_else makes copies when called on a const lvalue") {
      const auto v1 = e_int{make_unexpected(sec::runtime_error)};
    ...
    SECTION("or_else moves when called on a mutable rvalue") {
      ...
      check_eq(v1, make_unexpected(error{}));
      check_eq(v2, make_unexpected(sec::remote_linking_failed));
      auto v3 = std::move(v2).or_else(set_fallback);
      check_eq(v2, make_unexpected(error{}));
      check_eq(v3, 42);
    }
  ...
  SECTION("void value type") {
    ...
    SECTION("or_else makes copies when called on a const lvalue") {
      const auto v1 = e_void{make_unexpected(sec::runtime_error)};
      const auto v2 = v1.or_else(next_error);
      check_eq(v1, make_unexpected(sec::runtime_error));
      check_eq(v2, make_unexpected(sec::remote_linking_failed));
      const auto v3 = v2.or_else(set_fallback);
...
TEST("or_else leaves the expected unchanged when returning void") {
  SECTION("non-void value type") {
    ...
    auto inc = [&i](auto&& e) {
      ++i;
      return e_int{make_unexpected(e)};
    };
    auto v1 = e_int{make_unexpected(sec::runtime_error)};
  ...
  SECTION("void value type") {
    ...
    auto inc = [&i](auto&& e) {
      ++i;
      return e_void{make_unexpected(e)};
    };
    auto v1 = e_void{make_unexpected(sec::runtime_error)};
    auto v2 = v1.or_else(inc);                  // mutable lvalue
    const auto v3 = std::move(v2).or_else(inc); // mutable rvalue
...
TEST("or_else does nothing when called with a value") {
  SECTION("non-void value type") {
    auto uh_oh = [this](auto&& e) {
      fail("uh-oh called!");
      return e_iptr{make_unexpected(e)};
    };
    auto i = make_counted<counted_int>(1);
    auto v1 = e_iptr{std::in_place, i};
...
TEST("transform_error may replace the error or set a default") {
  SECTION("non-void value type") {
...
TEST("transform_error does nothing when called with a value") {
  SECTION("non-void value type") {
    ...
    auto i = make_counted<counted_int>(1);
    auto v1 = e_iptr{std::in_place, i};
    auto v2 = v1.transform_error(uh_oh);                  // mutable lvalue
    const auto v3 = std::move(v2).transform_error(uh_oh); // mutable rvalue
    const auto v4 = v3.transform_error(uh_oh);            // const lvalue
    auto v5 = std::move(v4).transform_error(uh_oh);       // const rvalue
    check_eq(v1, i);
    check_eq(v2, nullptr); // has been moved-from
    check_eq(v3, i);
    check_eq(v4, i);
