...
actor_system::dyn_spawn_impl(const std::string& name, message& args,
  ...
  if (name.empty())
    return expected<strong_actor_ptr>(unexpect, sec::invalid_argument);
  auto* fs = impl_->cfg->get_actor_factory(name);
  if (fs == nullptr)
    return expected<strong_actor_ptr>(unexpect, sec::unknown_type);
  ...
  if (check_interface && !assignable(res.second, *expected_ifs))
    return expected<strong_actor_ptr>(
      unexpect, sec::unexpected_actor_messaging_interface);
