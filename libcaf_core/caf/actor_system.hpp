...
public:
  ...
  /// There should be no user-spawned actors (including hidden ones)
  /// running upon destruction of the hosting actor system. Otherwise,
  /// `std::terminate()` is called.
  /// @sa `actor_system::await_all_actors_done()`
  ...
  // returns the number of currently running actors spawned from
  // this actor system, including unregistered hidden actors
  // generated by CAF runtime; provided for debugging purpose
  size_t num_running_actors() const;
...
private:
  ...
  void inc_running_actors();

  // decreases running-actors-count by one
  void dec_running_actors();

  std::atomic<actor_id> ids_;
  // number of all running actors spawned from this actor system
  // including unregistered hidden ones generated by CAF runtime
  std::atomic<size_t> num_running_actors_;
