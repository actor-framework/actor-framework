...
public:
  ...
  using intermediate_pseudo_tuple =
    typename detail::tl_apply<decayed_arg_types, detail::pseudo_tuple>::type;
...
typename std::enable_if<!std::is_base_of<match_case, F>::value,
                        std::tuple<trivial_match_case<F>>>::type
to_match_case_tuple(F fun) {
  return std::make_tuple(std::move(fun));
}

template <class MatchCase>
typename std::enable_if<std::is_base_of<match_case, MatchCase>::value,
                        std::tuple<const MatchCase&>>::type
...
const std::tuple<Ts...>& to_match_case_tuple(const std::tuple<Ts...>& x) {
  static_assert(
    detail::conjunction<std::is_base_of<match_case, Ts>::value...>::value,
