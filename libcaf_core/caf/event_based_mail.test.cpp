...
TEST("send fan_out_request messages that return a result") {
  ...
  SECTION("error response") {
    ...
    dispatch_messages();
    check_eq(*err, sec::unexpected_message);
    check_eq(*sum, 0);
  }
}
...
TEST("send fan_out_request messages that return two swapped values") {
  ...
  SECTION("error response with swapped values") {
    ...
    dispatch_messages();
    check_eq(*err, sec::unexpected_response);
    ...
    auto sender
      = sys.spawn([workers, swapped_values, err](event_based_actor* self) {
          self->mail(19, 23)
            ...
            .as_observable()
            .do_on_error([err](const error& x) { *err = x; })
            .for_each([swapped_values](const caf::cow_tuple<int, int>& value) {
              swapped_values->emplace_back(get<0>(value), get<1>(value));
            });
        });
    dispatch_messages();
    check_eq(*err, error{});
...
TEST("send urgent fan_out_request messages that return a result") {
  auto [self, launch] = sys.spawn_inactive();
  auto self_hdl = actor_cast<actor>(self);
  std::vector<actor> workers{
    make_server(sys, [](int x, int y) { return x + y; }),
  ...
  dispatch_messages();
  auto sum = std::make_shared<int>(0);
  SECTION("urgent fan_out_request with policy select_all") {
    self->mail(1, 2)
      .urgent()
      .fan_out_request(workers, infinite, policy::select_all_tag)
      .then([sum](std::vector<int> results) {
  ...
  SECTION("urgent delayed fan_out_request with policy select_all") {
    self->mail(1, 2)
      .urgent()
      .delay(1s)
      .fan_out_request(workers, infinite, policy::select_all_tag)
