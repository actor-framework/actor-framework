...
struct combine_latest_oracle {
  using output_type
    = decltype(std::declval<F&>()(std::declval<const Ts&>()...));
};

template <class F, class... Ts>
using combine_latest_output_t =
  typename combine_latest_oracle<F, Ts...>::output_type;

template <size_t Index>
using combine_index = std::integral_constant<size_t, Index>;

template <class T>
struct combine_input {
  using value_type = T;
...
/// Combines items from any number of observables using a combine function.
template <class F, class... Ts>
class combine_latest_sub : public subscription::impl_base {
public:
  // -- member types -----------------------------------------------------------

  using output_type = combine_latest_output_t<F, Ts...>;

  // -- constructors, destructors, and assignment operators --------------------

  combine_latest_sub(coordinator* parent, F fn, observer<output_type> out,
    ...
    for_each_input([this, &srcs](auto index, auto& input) {
      ...
      using fwd_impl = forwarder<value_type, combine_latest_sub, index_type>;
  ...
  bool buffered() {
    return fold([](auto&... x) { return (x.buf.has_value() && ...); });
  }

  // A combine latest op reached the end if all of its inputs reached the end.
  bool at_end() {
    return fold([](auto&... inputs) { return (inputs.completed && ...); });
  }

  // -- callbacks for the forwarders -------------------------------------------

  template <size_t I>
  void fwd_on_subscribe(combine_index<I> index, subscription sub) {
    if (out_) {
  ...
  template <size_t I>
  void fwd_on_complete(combine_index<I> index) {
    if (out_) {
      auto& input = at(index);
      if (input.sub)
  ...
  template <size_t I>
  void fwd_on_error(combine_index<I> index, const error& what) {
    if (out_) {
      if (!err_)
        err_ = what;
      auto& input = at(index);
  ...
  void fwd_on_next(combine_index<I> index, const T& item) {
    if (out_) {
      at(index).replace(item);
      push();
    }
  }
...
private:
  ...
  void push() {
    if (!buffered())
      return;
    demand_--;
    fold([this](auto&... x) { out_.on_next(fn_(x.latest()...)); });
  ...
  std::tuple<combine_input<Ts>...> inputs_;
...
/// Combines items from any number of observables using a combine function.
template <class F, class... Ts>
class combine_latest : public cold<combine_latest_output_t<F, Ts...>> {
public:
  // -- member types
  // -----------------------------------------------------------
  ...
  combine_latest(coordinator* parent, F fn, observable<Ts>... inputs)
  ...
  disposable subscribe(observer<output_type> out) override {
    using sub_t = combine_latest_sub<F, Ts...>;
...
/// Creates a new combine-latest operator from given inputs.
template <class F, class T0, class T1, class... Ts>
auto make_combine_latest(coordinator* parent, F fn, T0 input0, T1 input1,
                         Ts... inputs) {
  using output_type = combine_latest_output_t<F,                        //
                                              typename T0::output_type, //
                                              typename T1::output_type, //
                                              typename Ts::output_type...>;
  using impl_t = combine_latest<F,                        //
                                typename T0::output_type, //
                                typename T1::output_type, //
                                typename Ts::output_type...>;
