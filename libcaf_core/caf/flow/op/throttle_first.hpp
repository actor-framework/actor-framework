...
struct throttle_first_input_t {};

struct throttle_first_emit_t {};

/// The subscription for the `throttle_first` operator.
template <class T>
class throttle_first_sub : public subscription::impl_base {
public:
  // -- member types -----------------------------------------------------------
  ...
  throttle_first_sub(coordinator* parent, observer<output_type> out)
  ...
  void init(observable<input_type> vals, observable<select_token_type> ctrl) {
    using val_fwd_t
      = forwarder<input_type, throttle_first_sub, throttle_first_input_t>;
    using ctrl_fwd_t
      = forwarder<select_token_type, throttle_first_sub, throttle_first_emit_t>;
    auto fwd = parent_->add_child(std::in_place_type<val_fwd_t>, this,
                                  throttle_first_input_t{});
    ...
    if (running())
      ctrl.subscribe(parent_->add_child_hdl(std::in_place_type<ctrl_fwd_t>,
                                            this, throttle_first_emit_t{}));
  }

  // -- callbacks for the forwarders -------------------------------------------

  void fwd_on_subscribe(throttle_first_input_t, subscription sub) {
  ...
  void fwd_on_complete(throttle_first_input_t) {
    value_sub_.release_later();
  }

  void fwd_on_error(throttle_first_input_t, const error& what) {
    value_sub_.release_later();
    err_ = what;
  }
  ...
  void fwd_on_subscribe(throttle_first_emit_t, subscription sub) {
  ...
  void fwd_on_complete(throttle_first_emit_t) {
    ...
    if (state_ == state::running)
      err_ = make_error(sec::end_of_stream,
                        "throttle_first: unexpected end of the control stream");
    shutdown();
  }

  void fwd_on_error(throttle_first_emit_t, const error& what) {
    control_sub_.release_later();
    err_ = what;
    shutdown();
  }

  void fwd_on_next(throttle_first_emit_t, select_token_type) {
    if (!value_sub_) {
      shutdown();
      return;
    }
...
template <class T>
class throttle_first : public cold<T> {
public:
  // -- member types -----------------------------------------------------------
  ...
  throttle_first(coordinator* parent, input in, selector select)
  ...
  disposable subscribe(observer<output_type> out) override {
    auto ptr = super::parent_->add_child(
