...
TEST("push_back adds one element to the ring buffer") {
  int_buffer buf;
  print_info("add one element");
  buf.push_back(42);
  check(!buf.empty());
  check(!buf.full());
  check_eq(buf.size(), 1u);
  check_eq(buf.front(), 42);
  print_info("remove element");
  buf.pop_front();
  check(buf.empty());
  check(!buf.full());
  check_eq(buf.size(), 0u);
  print_info("fill buffer");
...
TEST("get_all returns all elements from the ring buffer") {
  ...
  check_eq(buf.front(), 0);
  print_info("drain elements");
  check_eq(fetch_all(), vector_type({0, 1, 2, 3, 4}));
  check(buf.empty());
  check(!buf.full());
  check_eq(buf.size(), 0u);
  print_info("add 60 elements (wraps around)");
