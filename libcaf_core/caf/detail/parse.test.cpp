...
#define CHECK_INVALID(type, str, code)                                         \
  check_eq(read<type>(str), unexpected{make_error(code)})
...
TEST("invalid timespans") {
  ...
  check_eq(read<timespan>("12nas"),
           unexpected{make_error(pec::unexpected_character)});
  check_eq(read<timespan>("34usec"),
           unexpected{make_error(pec::trailing_character)});
...
TEST("IPv4 address") {
  ...
  check_eq(read<ipv4_address>("256.0.0.1"),
           unexpected{make_error(pec::integer_overflow)});
}
