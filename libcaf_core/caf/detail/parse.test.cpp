...
expected<T> read(std::string_view str) {
  ...
  if (ps.code == pec::success)
    return result;
  return expected<T>{unexpect, ps.error()};
}
...
#define CHECK_INVALID(type, str, code)                                         \
  check_eq(read<type>(str), error_code{code})
...
TEST("invalid timespans") {
  check_eq(read<timespan>("12"), error_code{pec::unexpected_eof});
  check_eq(read<timespan>("12nas"), error_code{pec::unexpected_character});
  check_eq(read<timespan>("34usec"), error_code{pec::trailing_character});
  check_eq(read<timespan>("56m"), error_code{pec::unexpected_eof});
}
...
TEST("IPv4 address") {
  ...
  check_eq(read<ipv4_address>("256.0.0.1"), error_code{pec::integer_overflow});
}

TEST("IPv4 subnet") {
  check_eq(read<ipv4_subnet>("1.2.3.0/24"),
           ipv4_subnet(ipv4_address({1, 2, 3, 0}), 24));
  check_eq(read<ipv4_subnet>("1.2.3.0/33"), error_code{pec::integer_overflow});
}
...
TEST("IPv4 endpoint") {
  ...
  check_eq(read<ipv4_endpoint>("127.0.0.1:65536"),
           error_code{pec::integer_overflow});
}
...
TEST("IPv6 subnet") {
  ...
  check_eq(read<ipv6_subnet>("1::/129"), error_code{pec::integer_overflow});
}
