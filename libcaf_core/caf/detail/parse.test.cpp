...
#define CHECK_INVALID(type, str, code) check_eq(read<type>(str), unexpected{make_error(code)})
...
TEST("invalid timespans") {
  check_eq(read<timespan>("12"), unexpected{make_error(pec::unexpected_eof)});
  check_eq(read<timespan>("12nas"), unexpected{make_error(pec::unexpected_character)});
  check_eq(read<timespan>("34usec"), unexpected{make_error(pec::trailing_character)});
  check_eq(read<timespan>("56m"), unexpected{make_error(pec::unexpected_eof)});
}
...
TEST("IPv4 address") {
  ...
  check_eq(read<ipv4_address>("256.0.0.1"), unexpected{make_error(pec::integer_overflow)});
}

TEST("IPv4 subnet") {
  check_eq(read<ipv4_subnet>("1.2.3.0/24"),
           ipv4_subnet(ipv4_address({1, 2, 3, 0}), 24));
  check_eq(read<ipv4_subnet>("1.2.3.0/33"), unexpected{make_error(pec::integer_overflow)});
}
...
TEST("IPv4 endpoint") {
  ...
  check_eq(read<ipv4_endpoint>("127.0.0.1:65536"), unexpected{make_error(pec::integer_overflow)});
}
...
TEST("IPv6 subnet") {
  ...
  check_eq(read<ipv6_subnet>("1::/129"), unexpected{make_error(pec::integer_overflow)});
}
...
TEST("IPv6 endpoint") {
  ...
  check_eq(read<ipv6_endpoint>("127.0.0.1:65536"), unexpected{make_error(pec::integer_overflow)});
