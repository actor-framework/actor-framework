...
void basp_broker::proxy_announced(const node_id& nid, actor_id aid) {
  ...
  if (ptr == nullptr) {
    ...
    if (i == monitored_actors.end()) {
      monitor(ptr, [this, nid, entry](const error& reason) {
        if (monitored_actors.erase(entry) > 0)
...
void basp_broker::learned_new_node(const node_id& nid) {
  ...
  auto tmp = system().spawn<hidden>([=](event_based_actor* tself) -> behavior {
    auto lg = log::io::trace("");
    auto cache = std::make_shared<mail_cache>(tself, 10);
    return {
      [tself, cache](ok_atom, const std::string& /* key == "info" */,
      ...
      [cache](message msg) {
        // We skip messages until we receive the initial ok_atom.
