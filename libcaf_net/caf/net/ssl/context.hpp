...
ssl_ctx_chain(net::ssl::context& ctx, std::string_view descr, bool fn_res) {
  ...
  if (fn_res)
    ...
    return expected<context>{caf::unexpected{context::last_error_or_unexpected(descr)}};
}
...
ssl_ctx_chain(net::ssl::context& ctx, std::string_view arg_check_error,
  ...
  if ((!args && ...)) {
    ...
    return expected<context>{caf::unexpected{context::last_error_or_unexpected(fn_error)}};
  }
}
