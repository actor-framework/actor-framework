...
expected<std::pair<size_t, byte_span>> parse_chunk(byte_span input) {
  ...
  if (chunk.size() == input.size()) {
    ...
    if (input.size() >= 10)
      return {unexpect, sec::protocol_error, "Chunk size part is too long."};
  ...
  if (std::ranges::find(chunk, ';') != chunk.end())
    return {unexpect, sec::logic_error, "Chunk extensions not supported."};
  if (!std::ranges::all_of(chunk, isxdigit))
    return {unexpect, sec::protocol_error, "Chunk size decoding error."};
  if (chunk.size() > sizeof(size_t))
    return {unexpect, sec::protocol_error,
