...
#include "caf/net/web_socket/async_client_factory.hpp"
...
public:
  ...
  std::pair<scheme_and_port, scheme_and_port> schemes() const override {
    return {{"ws", defaults::net::http_default_port},
            {"wss", defaults::net::https_default_port}};
  }
...
void configure_handshake(async_client_config& config) {
  auto auth = config.endpoint.authority();
  auth.userinfo = std::nullopt;
  if (auth.port == defaults::net::http_default_port
      && config.endpoint.scheme() == "ws") {
    auth.port = 0;
  } else if (auth.port == defaults::net::https_default_port
             && config.endpoint.scheme() == "wss") {
    auth.port = 0;
  }
  config.hs.host(to_string(auth));
  auto endpoint = config.endpoint.path_query_fragment();
  if (endpoint.empty()) {
    endpoint = "/";
  }
  config.hs.endpoint(std::move(endpoint));
}

template <class Connection>
void start_connection(const_async_client_config_ptr config,
                      async::consumer_resource<frame> pull,
                      async::producer_resource<frame> push, Connection conn) {
  auto bridge = internal::make_ws_flow_bridge(pull, push);
  auto impl = client::make(handshake{config->hs}, std::move(bridge));
...
private:
  ...
  async::consumer_resource<frame> pull_;

  async::producer_resource<frame> push_;
};
...
async_client_factory_builder&&
async_client_factory_builder::protocols(std::string value) && {
  config_->hs.protocols(std::move(value));
  return std::move(*this);
}

async_client_factory_builder&&
async_client_factory_builder::extensions(std::string value) && {
  config_->hs.extensions(std::move(value));
  return std::move(*this);
}

async_client_factory_builder&&
async_client_factory_builder::header_field(std::string_view key,
                                           std::string value) && {
