...
/// Utility type for the fluent interface to asynchronous WebSocket clients.
class with_async {
public:
...
/// Entrypoint into the fluent WebSocket DSL for asynchronous operations.
class with_v2_t {
public:
...
/// Creates a new entrypoint into the fluent WebSocket DSL for asynchronous
/// client connections. Unlike `with(...)`, this API pushes the initial TCP
/// connection attempt to the async workers thread pool instead of blocking
/// the calling thread while establishing the connection.
