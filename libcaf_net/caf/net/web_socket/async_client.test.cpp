...
TEST("with_v2 connects asynchronously without blocking the caller") {
  ...
  auto parse_err = caf::parse(caf::detail::format("ws://127.0.0.1:{}", port),
                              endpoint);
  require(parse_err.empty());
  auto [pull, push] = make_spsc_buffer_resource<ws::frame>();
  auto [a2s_pull, a2s_push] = make_spsc_buffer_resource<ws::frame>();
  ws::with_v2(sys)
    .async()
...
TEST("with_v2 reports errors by closing the buffer with an error") {
  ...
  require(parse_err.empty());
  auto [s2a_pull, s2a_push] = make_spsc_buffer_resource<ws::frame>();
  auto [a2s_pull, a2s_push] = make_spsc_buffer_resource<ws::frame>();
  ws::with_v2(sys)
    .async()
    .connect(std::move(endpoint))
