...
/// Factory for launching asynchronous WebSocket clients. The
/// `async_client_factory` object is tied to a WebSocket endpoint that has
/// been configured via the `async_client_factory_builder` DSL.
...
public:
  ...
  /// caller.
  /// @returns the buffers for exchanging WebSocket frames with the connection.
...
public:
  ...
  [[nodiscard]] async_client_factory_builder&& protocols(std::string value) &&;

  /// @copydoc handshake::extensions
  [[nodiscard]] async_client_factory_builder&& extensions(std::string value) &&;

  /// @copydoc handshake::field
  [[nodiscard]] async_client_factory_builder&&
  header_field(std::string_view key, std::string value) &&;
  ...
  /// Creates a factory object that can be used to start WebSocket connections
