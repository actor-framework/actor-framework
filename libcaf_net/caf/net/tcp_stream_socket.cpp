...
make_connected_tcp_stream_socket(const uri::authority_type& node,
  ...
  auto port = node.port;
  if (port == 0)
    return {unexpect, sec::cannot_connect_to_node, "port is zero"};
  ...
  if (addrs.empty())
    return {unexpect, sec::cannot_connect_to_node, "empty authority"};
  for (auto& addr : addrs) {
    auto ep = ip_endpoint{addr, port};
    if (auto sock = make_connected_tcp_stream_socket(ep, timeout))
      return *sock;
  }
  return {unexpect, sec::cannot_connect_to_node, to_string(node)};
}
