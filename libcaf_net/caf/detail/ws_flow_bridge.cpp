...
public:
  ...
  ws_flow_bridge_impl(pull_t pull, push_t push)
...
public:
  ...
  explicit ws_flow_bridge_acceptor(detail::ws_conn_acceptor_ptr wca)
    : wca_(std::move(wca)) {
    // nop
  }
...
make_ws_flow_bridge(async::consumer_resource<net::web_socket::frame> pull,
  ...
  using impl_t = ws_flow_bridge_impl;
...
make_ws_flow_bridge(detail::ws_conn_acceptor_ptr wca) {
  using impl_t = ws_flow_bridge_acceptor;
