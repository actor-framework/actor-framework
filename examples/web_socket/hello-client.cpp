...
#include "caf/net/web_socket/with_v2.hpp"
...
int caf_main(caf::actor_system& sys, const config& cfg) {
  ...
  // Start an asynchronous connection to the server.
  auto [pull, push]
    = ws::with_v2(sys)
        .async()
        ...
        .max_retry_count(9)
        .start();
  // Spin up a worker to manage the connection.
  sys.spawn([hello, pull, push](caf::event_based_actor* self) {
    ...
    pull
      ...
      .do_on_error([self](const caf::error& what) {
        self->println("*** error while reading from the WebSocket: {}", what);
      })
      // Restrict how many messages we receive if the user configured a limit.
    ...
    // Send our hello message and wait until the server closes the socket. We
    // keep the connection alive by never closing the write channel.
