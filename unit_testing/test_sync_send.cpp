...
struct float_or_int : sb_actor<float_or_int> {
    behavior init_state = (on(atom("f")) >> [] { return 0.0f; },
                           on(atom("i")) >> [] { return 0; });
...
struct A : popular_actor {
    ...
    behavior make_behavior() override {
        return (on(atom("go"), arg_match) >> [=](const actor& next) {
                    CPPA_CHECKPOINT();
...
struct C : sb_actor<C> {
    behavior init_state;
    C() {
        init_state = (on(atom("gogo")) >> [=]()->atom_value {
            CPPA_CHECKPOINT();
            quit();
            return atom("gogogo");
        });
...
struct server : event_based_actor {
    ...
    behavior make_behavior() override {
        ...
        return (on(atom("idle"), arg_match) >> [=](actor worker) {
                    become(keep_behavior,
                           on(atom("request")) >> [=] {
                               forward_to(worker);
                               unbecome(); // await next idle message
                           },
                           on(atom("idle")) >> skip_message, others() >> die);
                },
                on(atom("request")) >> skip_message, others() >> die);
    }
...
void test_sync_send() {
    ...
    self->spawn<monitored + blocking_api>([](blocking_actor* s) {
        ...
        s->sync_send(foi, atom("f"))
            .await([&](int) { CPPA_UNEXPECTED_MSG(s); }, [&](float f) {
                 CPPA_CHECK_EQUAL(f, 0.f);
                 ++invocations;
             });
    ...
    auto await_success_message = [&] {
        self->receive(on(atom("success")) >> CPPA_CHECKPOINT_CB(),
                      on(atom("failure")) >>
                          CPPA_FAILURE_CB("A didn't receive sync response"),
                      on(non_normal_down_msg) >> [&](const down_msg& dm) {
            CPPA_FAILURE("A exited for reason " << dm.reason);
        });

    };
    self->send(self->spawn<A, monitored>(self), atom("go"),
               spawn<B>(spawn<C>()));
    ...
    self->timed_sync_send(self, std::chrono::milliseconds(50), atom("NoWay"))
        .await(on<sync_timeout_msg>() >> CPPA_CHECKPOINT_CB(),
               others() >> CPPA_UNEXPECTED_MSG_CB_REF(self));
    ...
    self->receive_for(i, 3)(
        ...
        on(atom("NoWay")) >> [] {
            CPPA_CHECKPOINT();
            CPPA_PRINT(
                "trigger \"actor did not reply to a "
    ...
    self->receive(others() >> CPPA_UNEXPECTED_MSG_CB_REF(self),
