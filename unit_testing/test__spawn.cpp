...
size_t test__spawn() {
    ...
    // create 2000 actors linked to one single actor
    ...
    auto pong_actor = spawn_link(pong, ping_actor);
    monitor(pong_actor);
    monitor(ping_actor);
    int i = 0;
    int flags = 0;
    ...
    receive_for(i, 4) (
        ...
        on(atom("DOWN"), pong_actor, arg_match) >> [&](std::uint32_t reason) {
            flags |= 0x02;
            CPPA_CHECK_EQUAL(reason, exit_reason::user_defined);
        },
        on(atom("DOWN"), ping_actor, arg_match) >> [&](std::uint32_t reason) {
            flags |= 0x04;
